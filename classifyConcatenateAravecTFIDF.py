...
class ArSentiment(object):
    def __init__(self, text_vect,embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        tweets = pd.read_csv("thesis-trainE.csv")
        tweets1 = pd.read_csv("thesis-testsetB.csv")
        ...
        #print('new data shape', a)
        #plt.plot(self.X_train[100])
        #plt.show()
        # with delete zeros form vector
        self.X_train2=np.concatenate((self.X_train,a),axis=1)
        self.X_test2=np.concatenate((self.X_test,b),axis=1)
        # without delete zeros
        #self.X_train2=np.concatenate((self.X_train,self.X_train1),axis=1)
        #self.X_test2=np.concatenate((self.X_test,self.X_test1),axis=1)
    ...
    def read_data(self, dataset_in):
        ...
        #tweets = pd.read_csv("D:\\NLP\\HateSpeechDetection\\DataSet\\clean_thesis-trainPureS.csv")
        #tweets = pd.read_csv("D:\\NLP\\HateSpeechDetection\\DataSet\\thesis-trainAUGS.csv")
        #tweets1 = pd.read_csv("D:\\NLP\\HateSpeechDetection\\DataSet\\clean_thesis-testsetBS.csv")
     ...
     def feature(self, words):
        ...
        for token in words:
            try:
                ...
                pass  # if a word is not in the embeddings' vocabulary discard it
                print(token)
        x_test = np.asarray(processed_token)
        print(len(x_test))         
        #input('sssssssssssssss')

        #processed_token= [ 'جمعه مباركه']  
        newtext=processed_token[0]
        for x in range(1, len(processed_token)):
            newtext=newtext+' '+processed_token[x]
        print(newtext)
        ...
        newtweet=[]
        newtext=[]
        for x in range(0, len(processed_token)):
            newtext=processed_token[x]
            newtweet.append(newtext)
        print('newtweet',newtweet)
        print('p',p)
        x_test = np.asarray(newtweet)
        print(x_test)
        tfidf_vec = text_vect.transform(p)
        print(tfidf_vec[0,:] )
        print(tfidf_vec.shape[0],tfidf_vec.shape[1])
        av=0.0
        for i in range(0, tfidf_vec.shape[1]):
            av=av+tfidf_vec[0,i]
        print(av,'      ',av/retrieved_words)    
        #input('wwwwwwwwwwwwww')
        #print(len(tfidf_vec))
        print(len(feature_vec))
        if tfidf_vec.shape[0]!=feature_vec.shape[0]:
            print(processed_token)
            print(feature_vec.shape[0])
            print(tfidf_vec.shape[0])
            #input('wwwwwwwwwwwwww')
        
        np.seterr(divide='ignore', invalid='ignore')
        feature_vec = np.divide(feature_vec, retrieved_words)
        feature_vec = np.multiply(feature_vec, av/retrieved_words)
        print(retrieved_words,len(words))
        print('+++++++++++++++++++++++++++++++++++++++')
        return feature_vec
    '''
