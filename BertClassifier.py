...
def get_ifidf_for_words(text):
    tfidf_matrix= tfIdfVectorizer.transform([' '.join(text)]).todense()
...
def preprocessing_for_bert(data):
    ...
    for sent in data:
        ...
        sent_ids = encoded_sent.get('input_ids')
        # tfidf = get_ifidf_for_words(sent)
        # sent_ids = np.concatenate((sent_ids, tfidf), axis=0)
...
def evaluate_roc(probs, y_true):
    ...
    plt.grid()
    plt.savefig("Results/ROC463gb.png", dpi=300, bbox_inches='tight')
    plt.show()
...
df = pd.read_csv("thesis-trainE.csv")
df.head()

train_text, val_text, train_label, val_label = train_test_split(df['FULLTEXT'], 
                                                                   df['ANOMALY'], 
                                                                   random_state=2812, 
                                                                   test_size=0.2, 
                                                                   stratify=df['ANOMALY'])

#dftest = pd.read_csv("D:\\MyProject\\cleantest500-2.csv")
dftest = pd.read_csv("thesis-testsetB.csv")
test_text = dftest['FULLTEXT']
test_labels = dftest['ANOMALY']

# Concatenate train data and test data
all_tweets = np.concatenate([df['FULLTEXT']])
...
# token_ids = list(preprocessing_for_bert([df['FULLTEXT'][0]])[0].squeeze().numpy())
# print('Original: ', df['FULLTEXT'][0])
...
train_labels = torch.tensor(train_label.tolist())
val_labels = torch.tensor(val_label.tolist())
...
# torch.save(bert_classifier, 'bert_classifier.cls')
...
for epoch in range(epochs):
    ...
    valid_loss, val_accuracy  = evaluate(bert_classifier, val_dataloader)
    #save the best model
