...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        param_test1 = {
            'subsample':[i/100.0 for i in range(80,100, 5)], # best 0.85
            'colsample_bytree':[i/100.0 for i in range(60,80, 5)] # BEST 0.7
        }
        gsearch1 = GridSearchCV(estimator = XGBClassifier(learning_rate = 0.1, n_estimators=5000, max_depth=3, min_child_weight=2, scale_pos_weight = 1), param_grid = param_test1, scoring='roc_auc',n_jobs=4,iid=False, cv=5)
