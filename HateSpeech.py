...
class HateSpeechDetection(object):
    ...
    def __init__(self, 
        ...
        #         coeff = [0.10, 0.10, 0.23, 0.32, 0.25]
        ...
        #         y_pred = pd.Series( [1 if row >= 0.5 else 0 for row in predict_proba])
        ...
        #         ])
        #         y_pred = pd.Series([1 if row >=  0.5 else 0 for row in newy_pred])
    ...
    def read_data(self, dataset_in):
        ...
        if self.isTest:
            with open('Results/tfidf-result', 'rb') as f:
                self.tfIdfVectorizer = pickle.load(f)
            self.feature_names = self.tfIdfVectorizer.get_feature_names()
        ...
        tfIdfVectorizer.fit_transform(dataset['FULLTEXT'])
        self.feature_names = tfIdfVectorizer.get_feature_names()
        self.tfIdfVectorizer = tfIdfVectorizer
        with open('Results/tfidf-result', 'wb') as f:
    ...
    def feature(self, words):
        ...
        tfidf = self.get_ifidf_for_words(words)
        return np.concatenate((feature_vec, tfidf), axis=0)
    ...
    def average_feature_vectors(self, examples, type_ = 'NaN'):
        ...
        """
        feature_vectors = np.zeros((len(examples), self.dimension+30), dtype = "float32")
    ...
    def get_ifidf_for_words(self, text):
        tfidf_matrix= self.tfIdfVectorizer.transform([' '.join(text)]).todense()
        ...
        if n == 30:
            return tfidf
        if n < 30:
            return np.pad(tfidf, (0, max(0, 30-n)), 'constant')
        else:
            return tfidf[:30]
    ...
    def classify(self, classifier = None, plot_roc = False):
        ...
        with open('Results/tfidf-'+classifier_name, 'wb') as f:
        ...
        y_pred = (predict_proba[:,1] >=  0.50).astype(bool) # set threshold as 0.5
    ...
    def predict(self, classifier = None, plot_roc = True):
        ...
        print('classifier_name',classifier_name)
        # load the classifier
        with open('Results/tfidf-'+classifier_name, 'rb') as f:
            classifier = pickle.load(f)
        ...
        # y_pred = (predict_proba[:,1] >=  0.5).astype(bool) # set threshold as 0.35
        self.predictions[classifier_name] = y_pred
        self.predict_proba[classifier_name] = predict_proba
        f1_score = metrics.f1_score(self.y_test, y_pred, zero_division = 0)
