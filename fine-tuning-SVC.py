...
grid = {
    'kernel': ['linear', 'rbf', 'poly'], # best poly
    'gamma': [0.1, 1, 10, 100], # best 100
    'C': [0.1, 1, 10, 100, 1000], # best 0.1
    'degree': [0, 1, 2, 3, 4, 5, 6] # best 2
    }
...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        # info("fitting ... ")
        bestModel, bestScore, allModels, allScores = pf.bestFit(SVC, paramGrid, self.X_train, self.y_train, self.X_test, self.y_test, metric = roc_auc_score, scoreLabel = "AUC")
