...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        param_test1 = {
            'gamma':[i / 10.0 for i in range(0, 5)], # best 0.4
        }
        gsearch1 = GridSearchCV(estimator = XGBClassifier(learning_rate = 0.1, n_estimators=5000, max_depth=3, min_child_weight=2, subsample = 0.8, colsample_bytree = 0.8, scale_pos_weight = 1), param_grid = param_test1, scoring='roc_auc',n_jobs=4,iid=False, cv=5)
