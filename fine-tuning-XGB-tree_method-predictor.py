...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        param_test1 = {
            ...
            # best params: {' predictor': 'auto', 'tree_method': 'gpu_hist'}
        }
        gsearch1 = GridSearchCV(estimator = XGBClassifier(learning_rate=1e-3, n_estimators=1500, max_depth=5, min_child_weight=1, colsample_bytree=0.85, subsample=0.65 , scale_pos_weight = 1, reg_alpha=1), param_grid = param_test1, scoring='roc_auc',n_jobs=4, cv=5)
        ...
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0 } 
        # best_param_test1 = {'learning_rate':1e-3, 'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0, 'learning_rate':1e-3 } 
        #input('ok')
