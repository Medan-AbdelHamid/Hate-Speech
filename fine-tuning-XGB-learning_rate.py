...
def get_test_seq():
    conn = sqlite3.connect('C:/Users/ASUS/Documents/Thesis/Code/Python/SoureCode/data-2020.db')
...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        param_test1 = {
            # 'learning_rate ':[1e-3, 1e-2, 1e-1] #  best 1e-3
            'learning_rate ':[1e-3, 1e-4, 1e-5] # best 1e-3
        }
        gsearch1 = GridSearchCV(estimator = XGBClassifier(n_estimators=5000, max_depth=3, min_child_weight=2, scale_pos_weight = 1, colsample_bytree=0.7, subsample=0.85, reg_alpha=1.7), param_grid = param_test1, scoring='roc_auc',n_jobs=4,iid=False, cv=5)
        gsearch1.fit(self.X_train, self.y_train)
        gsearch1.best_params_, gsearch1.best_score_
        print(gsearch1.best_score_)
        print(gsearch1.best_params_)
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1 }
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1 } FOR SECOND TIME
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2 } FOR SECOND TIME
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.7, 'subsample': 0.8 } FOR third TIME
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65 } FOR forth TIME
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0 } 
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0 } 
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0, 'learning_rate':1e-3 } 
        #input('ok')
