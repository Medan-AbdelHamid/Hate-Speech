...
dataset_file = 'C:/Users/ASUS/Documents/Thesis/Code/Python/SoureCode/thesis-trainset.csv'

# Number of trees in random forest
n_estimators1 = [int(x) for x in np.linspace(start = 10, stop = 100, num = 10)]
n_estimators2 = [int(x) for x in np.linspace(start = 200, stop = 2000, num = 10)]
n_estimators = n_estimators1 + n_estimators2 # best 2000
# Number of features to consider at every split
max_features = ['auto', 'sqrt'] # best sqrt
# Maximum number of levels in tree
max_depth = [int(x) for x in np.linspace(10, 110, num = 11)] # best 40
max_depth.append(None)
# Minimum number of samples required to split a node
min_samples_split = [int(x) for x in np.linspace(5, 45, num = 9)] # best 15
# Minimum number of samples required at each leaf node
min_samples_leaf = [1, 2, 4] # best 4
# Method of selecting samples for training each tree
bootstrap = [True, False] # best false
...
class ArSentiment(object):
    ...
    def plot_auc(self, estimator, estimator_name, neg, pos):
        ...
        plt.savefig("C:/Users/ASUS/Documents/Thesis/Code/Python/SoureCode/Results/ROC000.png", dpi=300, bbox_inches='tight')
        plt.grid()
...
if __name__ == "__main__":
    ...
    dataset_path = args.dataset if args.dataset else dataset_file 
    # run
    medyan = ArSentiment(embeddings_path, dataset_path, plot_roc=False)
