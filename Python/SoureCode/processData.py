...
theClassifiers = [
    ...
    SVC(kernel='poly', probability=True, degree=3, gamma=100, C=0.2),
    ...
    XGBClassifier(learning_rate = 1e-3 , n_estimators=5000, max_depth=3, min_child_weight=2, colsample_bytree=0.70, subsample=0.85, scale_pos_weight = 1, reg_alpha=1.7, predictor='auto', tree_method='gpu_hist', verbosity=0),
...
theClassifiersLevel2 = [
    ...
    LogisticRegressionCV(solver='liblinear', n_jobs=-1),
    LogisticRegression(n_jobs=-1, C=1, solver='saga'),
    MLPClassifier(hidden_layer_sizes=(100,100,100), activation='relu', solver='adam', alpha=0.05, learning_rate = 'invscaling', max_iter=1000),
    GaussianNB(),
]
...
hard_voting_cls = VotingClassifier(
    estimators=[('RandomForestClassifier', theClassifiers[0]),
                ('SGDClassifier', theClassifiers[1]),
                ('SVC', theClassifiers[2]),
                ('XGBClassifier', theClassifiers[3]),
                ('CatBoostClassifier', theClassifiers[4]),
                ('LogisticRegressionCV', theClassifiers[5]),
                ('LogisticRegression', theClassifiers[6]),
                ('MLPClassifier', theClassifiers[7]),
                # ('GaussianNB', theClassifiers[8])
                ],
    voting='hard')
...
classifiers = [soft_voting_cls, mlp]
# classifiers = [mlp, soft_voting_cls]
