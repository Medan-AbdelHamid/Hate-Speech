...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.80 , detailed=False):
        ...
        param_test1 = {
            ...
            # 'reg_alpha':[0.2, 0.5, 0.7, 1, 2, 5, 7, 10] 
            'reg_alpha':[0.8, 0.9, 1, 1., 1.5, 1.7] # best 1.7
        }
        gsearch1 = GridSearchCV(estimator = XGBClassifier(learning_rate = 0.1, n_estimators=5000, max_depth=3, min_child_weight=2, scale_pos_weight = 1, colsample_bytree=0.7, subsample=0.85), param_grid = param_test1, scoring='roc_auc',n_jobs=4,iid=False, cv=5)
        ...
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0 } FOR forth TIME
        # best_param_test1 = {'max_depth':5, 'min_child_weight':1, 'gamma':0.2, 'colsample_bytree': 0.85, 'subsample': 0.65, 'reg_alpha':1.0 } FOR forth TIME
        #input('ok')
