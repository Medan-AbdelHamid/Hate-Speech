...
"""
teordcloud.py is a file for printing most occurent words in our corpus
Author: Medyan Abdel Hamid
Date: Nov. 2020
"""
import re
import codecs
...
class ArSentiment(object):
    def __init__(self, embeddings_file=None, dataset_file=None, plot_roc=False, split=0.99 , detailed=False):
        """
        ...
        fullcorpus = train_tokens

        full_corpus = ' '
        for elem in  fullcorpus:
            for word in elem:
                if word not in stop_words:
                    full_corpus = full_corpus + ' ' + word
                    
        full_corpus = re.sub(r'[^\w\s]', '', full_corpus) 
        COUNTS = Counter(full_corpus.split())
        counts = {rtl(k):v for k, v in COUNTS.most_common(75)}
        print(COUNTS.most_common(3))

        wordcloud = WordCloud(font_path='tahoma.ttf', width=1200, height=800).generate_from_frequencies(counts)
...
if __name__ == "__main__":
    ...
    dataset_path = args.dataset if args.dataset else "thesis-trainAB.csv"
    # run
